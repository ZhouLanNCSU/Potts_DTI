...
c=40
  ...
  label_T0[(cc*3+1):c,]<-4
  Matrix_list=lapply(1:K, function(k) rwish(3, (k+1)*diag(3)/3))
  Matrix_list[[K]]<-rwish(3, 1.5*diag(3)/5)
  label_T1[1:cc,]<-1
...
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
dd=cbind((1*t(matrix(colMeans(result$Diff[3001:8000,])>0.5,40,40)))[,3:40],rep(0,40),rep(0,40))
```

```{r}
image(matrix(colMeans(result$Diff[3001:8000,]),40,40))
```
